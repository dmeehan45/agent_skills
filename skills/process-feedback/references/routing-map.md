...
|---|---|---|
| Inline turn reactions | `assistant_turn_feedback` (written by `src/components/app/MessageFeedback.tsx`, `source: 'inline'`) | lands / unsure / missed (plus `made_worse` from the outcome loop), optional note, and a `guidance_refs` blob joining the reaction back to the scripts / moments / KB chunks / prompt version that produced the turn (see `supabase/migrations/20260901120013_outcome_feedback.sql`) |
...
| Eval reviewer feedback | `eval_case_review` via the admin Evals → Review tab; exported by `src/lib/admin/evalFeedbackExport.ts` | the founding clinician's per-case verdicts (looks_right / suggest_shift) with plain-English notes |
| External, unstructured | the maintainer pastes it: a parent's email or DM, an app-store style review, a support thread, a user-interview or feedback-session transcript | verbatim words from a real reporter; capture source, date, and segment at intake |

## 2. Evidence sources for investigation (read-only)

- **Production database**: your Supabase project (read-only).
...
- **Prior art, so demand accumulates instead of fragmenting**: `qa/*.md`
  ...
  your repository, and the pending change-set queue.
...
| Paywall, pricing, upgrade | `src/paywall/` (`config.ts`, `entitlements.ts`), `src/components/app/UpgradeSheet.tsx`, `api/billing.ts`, Stripe webhook, `runtime_config` quota/trial values |
| Home | `src/pages/AppHomePage.tsx` (follow-up cards, memory snapshot) |
...
|---|---|---|
| `safety_trust` | the maintainer, immediately and first, with `docs/safety/trust-and-safety.md` in hand. Never a solo patch. | plain statement of what was reported, what the evidence shows, and the options; any harness fix that follows carries safety acceptance criteria and a fresh-session eval |
...
| `ux_confusion` | copy/flow fix via `frontend-polish` + `docs/design-system.md`; if the confusion is about what the product *is*, escalate to `adversarial-review` | the confused element quoted with file:line (or i18n key), what the parent expected it to mean |
| `content` | `content-rigor` (write or review mode) | the claim or passage, why it failed the reader, sourcing requirement |
| `expectation_mismatch` | fix the promise or the product, the maintainer chooses; `adversarial-review` owns the internal-incongruence framing; offer changes must update every mirror in one commit | the promising surface quoted with file:line vs the actual behaviour with its source |
| `missing_capability` | opportunity brief for the maintainer's roadmap decision; `needs_design` is an honest answer | restated underlying need (never the requested feature verbatim), demand evidence, Kano lens, options including "don't build" |
| `billing_pricing` | engineering fix for mechanics; the maintainer for anything touching price, value framing, or the offer | same as defect, plus the offer-mirror checklist |
...
- Anything that changes auth, schema, build tooling, env behaviour, API
  contracts, or more than three files: stop and ask the maintainer first (`AGENTS.md`).
